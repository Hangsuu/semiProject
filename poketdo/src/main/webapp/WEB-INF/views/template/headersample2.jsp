...
<html lang="ko">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="/static/css/basesample.css" />
   	...
   	<div style="display:inline-block; padding:10px">
		<c:choose>
			<c:when test="${empty sessionScope.memberId}">
				<a href="/member/login"><span style="color:white">로그인</span></a>
			</c:when>
			<c:otherwise>
				<a href="/member/logout"><span style="color:white">로그아웃</span></a>
			</c:otherwise>
		</c:choose>
     	</div>
	<div style="display:inline-block">
		<a href="/seal/list"><span style="color:white">인장뽑기</span></a>
	</div>
	<div style="display:inline-block">
		<c:if test="${sessionScope.memberId != null}">
			<a href="/message/receive"><span style="color:white">쪽지</span></a>
		</c:if>
	</div>
	<div style="display:inline-block">
		<c:if test="${sessionScope.memberLevel == '마스터'}">
			<a href="/admin/adminCheck"><span style="color:white">관리 페이지</span></a>
		</c:if>
	</div>
	<div style="display:inline-block">
		<a href="/member/mypage"><span style="color:white">마이페이지</span></a>
	</div>
</div>
      <%-- base.css --%>
    <div class="float-box">
